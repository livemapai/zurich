#!/usr/bin/env python3
"""
Nano Banana Two-Pass Tile Stitcher - Maximum quality seamless tiles.

TWO-PASS APPROACH:
    Pass 1: Generate all tiles with L-shaped context (establishes base style)
    Pass 2: Regenerate each tile with FULL 2×2 styled context from Pass 1
            - Styled neighbors from Pass 1 provide bidirectional context
            - Raw SOURCE tile ensures original geometry is preserved
            - Result: consistent colors + seamless edges + correct geometry

Why Two Passes?
    Pass 1 only has context from top-left (already processed tiles).
    Pass 2 has ALL neighbors styled, so each tile sees its actual surroundings.

Usage:
    python scripts/tile_pipeline/nano_stitcher_v2.py \\
        --source public/tiles/hybrid-golden_hour \\
        --style-name nano-winter-v2 \\
...
def call_gemini_with_reference(
    ...
    """Call Gemini API with context image + style reference, return styled image."""
    api_key = api_key or os.environ.get("GOOGLE_API_KEY") or os.environ.get("GEMINI_API_KEY")
    if not api_key:
        raise ValueError("API key required. Set GOOGLE_API_KEY environment variable.")

    context_b64 = image_to_base64(context_image)
    reference_b64 = image_to_base64(style_reference)

    url = f"{API_BASE}/{MODEL}:generateContent?key={api_key}"
    payload = {
        "contents": [{
            "parts": [
                {"inline_data": {"mime_type": "image/png", "data": context_b64}},
...
def create_style_reference(
    styled_tiles: dict[int, dict[int, Image.Image]],
    grid: dict[int, dict[int, Path]],
    max_size: int = 512,
) -> Image.Image:
    """
    Create a scaled-down thumbnail of all styled tiles as a style reference.
    This establishes THE color palette for Pass 2.
    """
    min_x, min_y, max_x, max_y = get_grid_bounds(grid)
    width = (max_x - min_x + 1) * TILE_SIZE
    height = (max_y - min_y + 1) * TILE_SIZE

    # Create full-size composite
    composite = Image.new("RGB", (width, height), color=(128, 128, 128))

    for y in sorted(styled_tiles.keys()):
        for x in sorted(styled_tiles[y].keys()):
            tile = styled_tiles[y][x]
            px = (x - min_x) * TILE_SIZE
            py = (y - min_y) * TILE_SIZE
            composite.paste(tile.resize((TILE_SIZE, TILE_SIZE)), (px, py))

    # Scale down to max_size while preserving aspect ratio
    scale = min(max_size / width, max_size / height)
    new_size = (int(width * scale), int(height * scale))
    reference = composite.resize(new_size, Image.LANCZOS)

    return reference


def generate_with_reference(
    context_2x2: Image.Image,
    style_reference: Image.Image,
    position: str,
    user_prompt: str,
    temperature: float = 0.3,
    ...
    """
    Generate a tile using 2×2 context + style reference.

    context_2x2: 1024×1024 image with 3 styled tiles + 1 raw tile
    style_reference: Thumbnail of the full styled grid (color palette reference)
    position: Which corner has the raw tile ('tl', 'tr', 'bl', 'br')

    Returns the styled tile for the specified position.
    """
    position_names = {
        'tl': 'TOP-LEFT',
        'tr': 'TOP-RIGHT',
...
IMAGE 1: A 2×2 grid of map tiles (1024×1024 total).
IMAGE 2: A style reference showing the color palette to match.

In the 2×2 grid, three tiles are already styled. The {position_names[position]} tile is RAW and needs styling.

Style the {position_names[position]} tile to match the colors from IMAGE 2 and blend with neighbors.

Style: {user_prompt}

Return exactly 1024×1024 pixels."""

    result = call_gemini_with_reference(
        context_2x2, style_reference, prompt, GRID_2X2_SIZE, temperature, api_key
    )
    ...
    return result.crop(positions[position])
...
def process_grid(
    ...
    # ==========================================================================
    # PASS 2: Regenerate with style reference + full 2×2 context
    # ==========================================================================

    print(f"\n  Creating style reference from Pass 1...")
    style_reference = create_style_reference(styled, grid)
    print(f"    Style reference: {style_reference.size}")

    print(f"\n  Pass 2: Regenerate with style reference + 2×2 context")
    ...
    for y in all_ys:
        for x in sorted(grid[y].keys()):
            # Get all 4 neighbors from Pass 1 (styled)
            ...
            # Determine best 2×2 configuration based on available neighbors
            # We need 3 styled neighbors + 1 raw tile in a 2×2 grid

            context_2x2 = None
            position = None

            # Try to find best configuration (prefer having more context)
            if styled_above and styled_left and styled.get(y - 1, {}).get(x - 1):
                # Can use top-left, top, left + raw at bottom-right
                context_2x2 = stitch_2x2(
                    styled.get(y - 1, {}).get(x - 1),  # above-left
                    styled_above,                       # above
                    styled_left,                        # left
                    raw_tile                            # raw at bottom-right
                )
                position = 'br'

            elif styled_above and styled_right and styled.get(y - 1, {}).get(x + 1):
                # Can use top, top-right, raw at bottom-left, right
                context_2x2 = stitch_2x2(
                    styled_above,                       # above
                    styled.get(y - 1, {}).get(x + 1),  # above-right
                    raw_tile,                           # raw at bottom-left
                    styled_right                        # right
                )
                position = 'bl'

            elif styled_below and styled_left and styled.get(y + 1, {}).get(x - 1):
                # Can use left, raw at top-right, below-left, below
                context_2x2 = stitch_2x2(
                    styled_left,                        # left
                    raw_tile,                           # raw at top-right
                    styled.get(y + 1, {}).get(x - 1),  # below-left
                    styled_below                        # below
                )
                position = 'tr'

            elif styled_below and styled_right and styled.get(y + 1, {}).get(x + 1):
                # Can use raw at top-left, right, below, below-right
                context_2x2 = stitch_2x2(
                    raw_tile,                           # raw at top-left
                    styled_right,                       # right
                    styled_below,                       # below
                    styled.get(y + 1, {}).get(x + 1)   # below-right
                )
                position = 'tl'

            if context_2x2 is None:
                # Not enough neighbors for 2×2, skip Pass 2 for this tile
                print(f"    [{x},{y}] Skip (insufficient neighbors)")
                continue

            print(f"    [{x},{y}] Pass 2 ({position})...", end=" ", flush=True)
            start = time.time()

            try:
                new_styled = generate_with_reference(
                    context_2x2, style_reference, position, user_prompt, temperature, api_key
                )
                styled[y][x] = new_styled
                save_tile(x, y, new_styled)
                print(f"✓ ({time.time() - start:.1f}s)")
